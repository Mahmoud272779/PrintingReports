...
{
    public class EditEmployeeGroupsHandler : IRequestHandler<EditEmployeeGroupsRequest, ResponseResult>
    {
        ...
        public EditEmployeeGroupsHandler(IRepositoryCommand<Domain.Entities.Process.AttendLeaving.EmployeesGroup> EmployeeGroupsCommand, IRepositoryQuery<Domain.Entities.Process.AttendLeaving.EmployeesGroup> EmployeeGroupsQuery)
        {
            _EmployeeGroupsCommand = EmployeeGroupsCommand;
            _EmployeeGroupsQuery = EmployeeGroupsQuery;
        }

        public async Task<ResponseResult> Handle(EditEmployeeGroupsRequest request, CancellationToken cancellationToken)
        {
            ...
            var element = await _EmployeeGroupsQuery.GetByIdAsync(request.Id);
            if(element == null)
                return new ResponseResult
                {
                    Result = Result.Failed,
            ...
            var saved = await _EmployeeGroupsCommand.UpdateAsyn(element);
            return new ResponseResult
            {
